...
namespace AspNetCoreHero.Boilerplate.Web.Areas.Admin.Controllers
{
    ...
    public class UserController : BaseController<UserController>
    {
        ...
        {
            var ticketTypesList = await _mediator.Send(new GetAllTicketTypesQuery());
            var viewModel = ticketTypesList.Data.Adapt<List<TicketTypeViewModel>>();
            var ticketTyes = new SelectList(viewModel, nameof(TicketTypeViewModel.Id), nameof(TicketTypeViewModel.Name), null, null);
            var supportViewModel = new SupportViewModel
            {
                TicketTypes = ticketTyes
            };

            return new JsonResult(new { isValid = true, html = await _viewRenderer.RenderViewToStringAsync("_CreateSupport", supportViewModel) });
        }
